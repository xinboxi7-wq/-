...
- 第 4 页升级为触摸板、陀螺仪、灯带与电量、自适应扳机四分区原生 WPF 页面。
- 新增 Contact ID 生命周期、双指独立轨迹、1600 点上限、5 秒录制、24×12 覆盖图和五步触摸检测。
- 新增三轴正负响应、中断、跳变、冻结、零偏/噪声诊断；保留 Mahony 风格融合并可切换纯陀螺积分姿态。
- 新增按哈希设备键保存的静止校准、灵敏度和平滑度配置。
- 灯带实际 RGB 状态未解析；灯带与自适应扳机 USB / 蓝牙输出均待实机验证，控件保持禁用且不发送报告。
...
- 在现有 `IControllerRumbleService` 上增加统一能力模型、14 个数据化预设和独立 25 Hz 时间线播放器。
- 新增原生 WPF 双通道曲线编辑、暂停、紧急停止、自定义预设和设备级感知校准。
- 新增哈希设备配置、损坏配置恢复及漂移 / 体感 / 完整检测静止阶段互斥。
- DualSense 仍只开放基础兼容震动；高级触觉与自适应扳机明确保持未开放。
